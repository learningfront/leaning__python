...
def process_user_choice(choice, person):
    if choice == 1:
        ...
        try:
            count = round(float(input('Сумма к снятию:')), 2)
        except ValueError:
            print('Нафига ты вводишь не цифры, ало дурак')
            return False
        if count <= 0:
            print('Ты дурак, какие отрицательные числа, введи нормально')
        else:
            print(withdraw_money(person, count))

patern_for_card = '^\d{16}$'
patern_for_code = '^\d{4}$'
...
def start():
    try:
        ...
        print('Бля я же сказал только номер карты и пароль через пробел')
    except ValueError:
        print('Бля я же сказал только номер карты и пароль через пробел')
    if re.match(patern_for_card, card_number) is not None:
        card_number = int(card_number)
    else:
        print('Я же просил номер карты только 16 символов')


    if re.match(patern_for_code, pin_code) is not None:
        pin_code = int(pin_code)
    else:
        print('Я же просил пароль 4 символа')
